function [metric, rectangleCellOut, resid, residAssocIdxs] = ...
    cim_v3(x, y, varargin)
        ...
        for ax2mmCfgIdx=1:length(ax2minmaxCfg)
            ...
            while(scanincr>=minscanincr)
                ...
                [metricVecTmp, numPtsVecTmp, rectangles] = ...
                    scanForDep(ax1pts,ax2pts,ax2min,ax2max,scanincr,diffthresh,alpha);
...
if(nargout>1)
    ...
    rectangleCellOut = rectangleCell{1}{maxIIvec(metricMaxIdx)};
...
%             matchPts = [ax1pts(matchIdxs) ax2pts(matchIdxs)];
            [matchPts, matchIdxs] = getPointsWithinBounds(ax1pts, ax2pts, ax1min, ax1max, ax2min, ax2max);

            XX = [ones(size(matchPts,1),1) matchPts(:,1)];
            zz = XX\matchPts(:,2);
...
function [metricVec, numPtsVec, rectangles] = scanForDep(ax1pts, ax2pts, ax2min, ax2max, scanincr, diffthresh, alpha)
