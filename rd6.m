function [borderPts] = rd6(ax1pts, ax2pts, scanincr)
...
while ax1max<=1
    ...
    if(numPts>=2)   % make sure we have enough points to compute the metric
        ...
        sigma_sq = (2*(2*numPts+5))./(9*numPts.*(numPts-1));
        
        zsc  = metricRectangle./sqrt( sigma_sq );   
        if(newRectangle)
            newRectangle = 0;
        else
            % make it a symmetric percentage change
            percentageChange = (metricRectangle-metricRectanglePrev)/((metricRectanglePrev+metricRectangle)/2);
