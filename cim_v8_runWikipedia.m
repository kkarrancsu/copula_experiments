...
xContinuous = rand(M, 1)*2-1; % uniform between +/- 1
yContinuous = rand(M, 1)*2-1;
...
cimBias = cim_v8a_cc_mex(xContinuous, yContinuous, minScanIncr);
...
xContinuous = rand(M,1)*2-1;
yContinuous = 2*xContinuous.^2 + (rand(M,1)*2-1)*spread;
dat = [xContinuous yContinuous];
xy{xyIdx} = dat; xxyy{xyIdx} = [discretizeRv(dat(:,1),numDiscreteIntervals)' discretizeRv(dat(:,2),numDiscreteIntervals)'];
xyIdx = xyIdx + 1;

xContinuous = rand(M,1)*2-1;
yContinuous = (xContinuous.^2 + rand(M,1)/2 * spread) .* (2*binornd(1,0.5,M,1)-1);
dat = [xContinuous yContinuous];
xy{xyIdx} = dat; xxyy{xyIdx} = [discretizeRv(dat(:,1),numDiscreteIntervals)' discretizeRv(dat(:,2),numDiscreteIntervals)'];
xyIdx = xyIdx + 1;

xx = (rand(M,1)*2-1);
yContinuous = cos(xx*pi) + randn(M,1)*1/8 * spread;
xContinuous = sin(xx*pi) + randn(M,1)*1/8 * spread;
dat = [xContinuous yContinuous];
xy{xyIdx} = dat; xxyy{xyIdx} = [discretizeRv(dat(:,1),numDiscreteIntervals)' discretizeRv(dat(:,2),numDiscreteIntervals)'];
xyIdx = xyIdx + 1;

spread = 0.1;
xy1 = mvnrnd([3 3], [1 spread; spread 1], M/4);
xy2 = mvnrnd([-3 3], [1 spread; spread 1], M/4);
xy3 = mvnrnd([3 -3], [1 spread; spread 1], M/4);
xy4 = mvnrnd([-3 -3], [1 spread; spread 1], M/4);
dat = [xy1; xy2; xy3; xy4];
xy{xyIdx} = dat; xxyy{xyIdx} = dat;     % TODO: fix discrete!

xContinuous = rand(M,1);
...
%     h = subplot(3,3,ii);
    ...
    cimValContinuous = cim_v8a_cc_mex(xContinuous, yContinuous, minScanIncr);
    ...
    cimValDiscrete = cim_v8a_cc_mex(xDiscrete, yDiscrete, minScanIncr);
...
%     ccorrValDiscrete = ccor(xDiscrete,yDiscrete);
    ccorrValDiscrete = 0;   % glitches b/c it is only meant for continuous
