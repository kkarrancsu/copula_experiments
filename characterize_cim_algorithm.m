...
if(~exist('masterCfgRun') || (masterCfgRun==1 && runConvergence) )
    ...
    for M=MVec
        ...
        % save the data
        if(ispc)
            save(sprintf('C:\\Users\\Kiran\\ownCloud\\PhD\\sim_results\\independence\\cim_convergence_%s_M_%d.mat', fnameStr, M));
        elseif(ismac)
            save(sprintf('/Users/Kiran/ownCloud/PhD/sim_results/independence/cim_convergence_%s_M_%d.mat', fnameStr, M));
        else
            save(sprintf('/home/kiran/ownCloud/PhD/sim_results/independence/cim_convergence_%s_M_%d.mat', fnameStr, M));
        end
    end
end
%% Plot the results
