function [metric,regionRectangle] = cim_v2_hybrid_cc(x, y, minScanIncr, alpha)
...
while ax1max<=1
    ...
    if(numPts>=2)   % make sure we have enough points to compute the metric
        ...
        metricRectangle = min(abs(taukl_cc( matchPts(:,1),matchPts(:,2),0,1,0)),1);  % investigate why we need max
...
if(metricRectanglePrev<0)
    metricVec(rectanglesIdx) = abs(taukl_cc( ax1pts,ax2pts,0,1,0 ));
    numPtsVec(rectanglesIdx) = length(ax1pts)-sum(numPtsVec(1:rectanglesIdx));
